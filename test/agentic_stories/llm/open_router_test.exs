defmodule AgenticStories.LLM.OpenRouterTest do
  use ExUnit.Case, async: true

  alias AgenticStories.LLM.{OpenRouter, Request, Response}
  ...
  test "posts an OpenAI-compatible request to /v1/chat/completions" do
    ...
    assert {:ok, %Response{} = response} = OpenRouter.chat(request())
  ...
  test "renders blocks as text parts, cache: true as a cache_control breakpoint" do
    ...
    assert {:ok, %Response{text: "ok"}} = OpenRouter.chat(request)
  end
  ...
  test "omits the system message when the request has none" do
    ...
    assert {:ok, %Response{text: "ok"}} = OpenRouter.chat(request(system: nil))
  end
  ...
  test "returns an error tuple for non-200 responses" do
    ...
    assert {:error, {:http_error, 429, _body}} = OpenRouter.chat(request())
  end
end
